...
```{r, echo=FALSE}
x <- seq(0.2, 1.2, length = 100)
y <- sapply(x, FUN = function(x){3*x+(1/x)})
y2 <- sapply(x, FUN = function(x){4})

xl <- seq(1/3, 1, length=100)
yl1 <- sapply(xl, FUN = function(x){3*x+(1/x)})
yl2 <- sapply(xl, FUN = function(x){4})
...
grid()
points(x, y2, type='l')
polygon(xx, yy, col="#55aaff", border = "#225599")
text(0.6, 3.75, labels = "M")
