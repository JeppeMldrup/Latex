...
b. Tegn grafen for $P$ i et passende interval, og bestem den øvre grænse for antallet af guppyer i akvariet
   ...
   ```{r, echo=FALSE, dev='pdf', fig.cap='grafen for funktionen P(t)'}
   t = seq(0, 60, length = 200)
   func = function(t){150/(1+(11.5*exp(-0.225*t)))}
   p = sapply(t, FUN = func)

   plot(t, p, type='l', xlab = "antal uger", ylab = "antal guppyer", ylim = c(0, 150))
   grid()
   ```
   \pagebreak
...
a. Bestem en ligning for hver af disse tangenter
   ...
   ```{r, echo = FALSE, dev='pdf', fig.cap='Tangenter'}
   a1 = 5.82842712475
   a2 = 0.17157287525
   x = seq(-3, 6, length=200)
   func = function(x){-x^2+3*x-2}
   func1 = function(x){a1*x}
   func2 = function(x){a2*x}
   y = sapply(x, FUN=func)
   y1 = sapply(x, FUN=func1)
   y2 = sapply(x, FUN=func2)
   plot(x, y, type = 'l', ylim = c(-10, 2))
   lines(x, y1)
   lines(x, y2)
   abline(h=0)
   abline(v=0)
